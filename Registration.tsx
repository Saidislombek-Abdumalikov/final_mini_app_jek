        ...
        <div className="fixed inset-0 bg-slate-900 z-[100] flex flex-col items-center justify-center p-8 animate-fade-in">
            ...
            <form onSubmit={handleAdminSubmit} className="w-full max-w-xs space-y-4">
                <input 
                    ...
                    className="w-full p-4 bg-slate-800 border border-slate-700 rounded-2xl focus:border-blue-500 outline-none text-center text-xl text-white tracking-widest placeholder:text-slate-600"
                    autoFocus
                />
                {error && <p className="text-red-400 text-sm text-center font-medium">{error}</p>}
                <button type="submit" className="w-full py-4 bg-blue-600 text-white rounded-2xl font-bold shadow-xl shadow-blue-900/20 active:scale-95 transition-transform">Kirish</button>
      ...
      <div className="w-full max-w-sm bg-white p-8 rounded-[40px] shadow-soft">
        ...
        <div className="text-center mb-8">
           <div 
             ...
             className="w-20 h-20 bg-gradient-to-tr from-primary to-primary-dark rounded-3xl mx-auto flex items-center justify-center text-white font-bold text-4xl shadow-glow mb-6 rotate-3 cursor-pointer select-none active:scale-90 transition-transform"
           >
              J
           </div>
           <h1 className="text-2xl font-black text-primary-dark">JEK KARGO</h1>
