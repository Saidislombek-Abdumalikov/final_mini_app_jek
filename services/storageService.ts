...
const DB_KEY_USER_TRACKS_PREFIX = 'jek_kargo_user_tracks'; 
const DB_KEY_SETTINGS = 'jek_kargo_settings';
const DB_KEY_USER_PROFILE = 'jek_kargo_user_profile';
const DB_KEY_ALL_REGISTRATIONS = 'jek_kargo_admin_registrations';
...
export const fetchAllClientsFromSheet = async (): Promise<ClientActivity[]> => {
    ...
    try {
        ...
        for (let i = 1; i < rows.length; i++) {
            ...
            // Allow JEK, JK, TT, TOP prefixes - prioritize JK
            const clientIdIdx = cols.findIndex(c => /^(JK|JEK|TT|TOP)/i.test(c.trim()));
            // Look for phone
