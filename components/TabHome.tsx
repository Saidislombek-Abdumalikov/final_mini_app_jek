...
const TabHome: React.FC<TabHomeProps> = ({ onNavigate }) => {
  ...
  const getAddressData = () => {
      ...
      return { receiver, phone, address, smartString };
  };
  ...
  return (
    <div className="space-y-4 pb-32 animate-fade-in">
      ...
      {/* Address Card (Clickable) - COMPACT */}
      <div 
        onClick={() => setShowAddressModal(true)}
        className="w-full relative overflow-hidden bg-gradient-to-br from-[#0066CC] to-[#004499] rounded-[24px] p-5 text-white shadow-xl shadow-blue-900/20 group cursor-pointer active:scale-[0.98] transition-all hover:shadow-2xl hover:shadow-blue-900/30"
      >
        <div className="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full -mr-16 -mt-16 blur-3xl"></div>
        <div className="absolute bottom-0 left-0 w-32 h-32 bg-blue-400/20 rounded-full -ml-8 -mb-8 blur-2xl"></div>
        ...
        <div className="relative z-10">
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                 <span className="block font-bold leading-none text-white/90 text-sm">JEK ID</span>
                 <span className="text-[9px] text-blue-200 font-medium">Shaxsiy Raqam</span>
              </div>
            </div>
          ...
          <div className="mb-4 text-center">
            <p className="text-4xl font-mono font-black tracking-widest text-shadow-sm leading-none">{clientId.replace(/^(JEK|JK|TT|TOP)/, '')}</p>
          </div>

          <div className="flex items-center justify-between border-t border-white/10 pt-3">
            <div className="flex items-center gap-2 text-blue-100 text-[10px] font-bold uppercase tracking-wide">
                      ...
                      <div className="bg-white p-5 rounded-[24px] shadow-sm border border-gray-200">
                          ...
                          <div className="relative group">
                              ...
                              <button 
                                ...
                                className={`absolute top-2 right-2 p-2 rounded-lg transition-all shadow-sm border active:scale-95 ${copyFeedback ? 'bg-green-50 border-green-200 text-green-600' : 'bg-white border-gray-200 text-primary hover:bg-blue-50'}`}
                              >
                                 {copyFeedback ? (
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
              ...
              <div className={`bg-white w-full max-w-sm rounded-[32px] p-8 shadow-2xl z-10 relative transform transition-all duration-300 max-h-[85vh] overflow-y-auto ${isClosing ? 'scale-90 opacity-0' : 'scale-100 opacity-100 animate-slide-up'}`}>
                  ...
                  <div className="text-center mb-8">
                      <div className={`w-20 h-20 rounded-3xl mx-auto flex items-center justify-center text-4xl mb-5 shadow-sm ${infoModal === 'AVIA' ? 'bg-blue-50 text-primary' : 'bg-green-50 text-green-500'}`}>
                          {infoModal === 'AVIA' ? '✈️' : '🚛'}
                      </div>
